...
def run_document_intelligence(image_path):
    ...
    with open(image_path, "rb") as f:
        poller = client.begin_analyze_document(
            "prebuilt-layout",
            body=f,
            # output_content_format=DocumentContentFormat.MARKDOWN,  # enables markdown on overall content section in output
            features=["ocrHighResolution"],
            # output=[AnalyzeOutputOption.FIGURES],  # explicitly requests figure metadata
        )
...
def main():
    ...
    output_dir = Path(f"output/pipeline results old/{date_str}")
    ...
    for img_path in INPUT_DIR.glob("*.*"):
        ...
        step_half_path = output_dir / f"{base_name}_step_half_spatial_{timestamp}.json"
        with open(step_half_path, "w") as f:
...
#                 "id": f"fig_{j}",
#                 "polygon": (
#                     fig.get("boundingRegions")[0].get("polygon")
...
#         with open(
#             OUTPUT_DIR / f"{img_path.stem}_step2_geometry_{timestamp}.json", "w"
#         ) as f:
...
#         with open(
#             OUTPUT_DIR / f"{img_path.stem}_step3_spatial_{timestamp}.json", "w"
#         ) as f:
...
#         with open(
#             OUTPUT_DIR / f"{img_path.stem}_step4_final_{timestamp}.json", "w"
#         ) as f:
#             json.dump(final_json, f, indent=4)
